...
def lmp_traj_info(lmp_traj_file, lmp_log_file, return_frames_num_fic=False):
  ...
  line_num = file_tools.grep_line_num('Step ', os.path.abspath(os.path.expanduser(lmp_log_file)), os.getcwd())
  if ( line_num == 0 ):
    log_info.log_error('File error: %s file error, please check' %(os.path.abspath(os.path.expanduser(lmp_log_file))))
    exit()
  else:
    a_int = line_num[0]
  line_num = file_tools.grep_line_num('Loop time', os.path.abspath(os.path.expanduser(lmp_log_file)), os.getcwd())
  if ( line_num == 0 ):
    whole_line_num = len(open(os.path.abspath(os.path.expanduser(lmp_log_file)), 'r').readlines())
  ...
  whole_line_num = len(open(os.path.abspath(os.path.expanduser(lmp_traj_file)), 'r').readlines())
...
def read_lmp_log_traj(lmp_traj_file, lmp_log_file, atom_label={}, frames=[], ene_return=False, \
  ...
  line_num = file_tools.grep_line_num('Step', lmp_log_file, os.getcwd())
  line = linecache.getline(lmp_log_file, line_num[0])
  line_split = data_op.split_str(line, ' ', '\n')
  ...
  energy_fic = []
  for i in frames_fic:
    line_log_i = linecache.getline(lmp_log_file, int((i-start_id)/each)+line_num[0]+1)
