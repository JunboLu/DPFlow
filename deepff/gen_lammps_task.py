...
def gen_lmpmd_task(lmp_dic, work_dir, iter_id, atom_mass_dic, tot_atoms_type_dic, dp_version):
  ...
  sys_num, atoms_type_multi_sys, atoms_num_tot, use_bias_tot = process.get_md_sys_info(lmp_dic, tot_atoms_type_dic)
  ...
  for i in range(sys_num):
    ...
    for j in range(len(temp)):
      for k in range(len(pres)):
        ...
        if vary_md_step:
          final_nsteps = int(lmp_dic['nsteps'])
          nsteps = 25000*(2**iter_id)
          if ( nsteps > final_nsteps ):
            nsteps = final_nsteps
          else:
            new_lmp_dic['thermo_freq'] = math.ceil(nsteps/1000)
            new_lmp_dic['dump_freq'] = math.ceil(nsteps/1000)
        else:
          nsteps = int(lmp_dic['nsteps'])

        new_lmp_dic['nsteps'] = nsteps
        new_lmp_dic.pop('change_init_str')
        new_lmp_dic.pop('vary_md_step')
        ...
        if 'plumed_file' in lmp_dic[sys]:
          ...
          for l in range(1, temp_line_num[0], 1):
            ...
            plumed_file_task.write(line)
          plumed_file_task.write(' TEMP=%f\n' %(temp[j]))
          for l in range(temp_line_num[0]+1, whole_line_num+1, 1):
            line = linecache.getline(plumed_file_name, l)
            plumed_file_task.write(line)
          linecache.clearcache()
          plumed_file_task.close()
          md_in_file.write('fix             1 all plumed plumedfile %s outfile plumed.log\n' % (plumed_file_task_name))
          if ( md_type == 'nvt' ):
...
def gen_lmpfrc_file(work_dir, iter_id, atom_mass_dic, atoms_num_tot, \
                    atoms_type_multi_sys, use_bias_tot, active_type, dp_version):
  ...
  Args:
    ...
    use_bias_tot: 1-d bool list
      use_bias_tot is whether using metadynamics for different systems.
    active_type: string
  ...
  for i in range(sys_num):
    ...
    for j in range(task_num):
      ...
      atom_file_name = call.call_returns_shell(lmp_sys_task_dir, cmd)
    ...
    for i in range(frag_traj_num):
      ...
      for j in range(frames_num_fic):
        ...
        if ( frames_num_stat > frames_num ):
          ...
          if ( len(line_split) != 0 ):
            if ( line_split[0] == 'WARNING:' or \
                 (len(line_split) == log_id_num and data_op.eval_str(line_split[0]) == 1) ):
              tot_log_file.write(line)
