...
def gen_deepmd_model_task(deepmd_dic, work_dir, iter_id, init_train_data, numb_test, descr_seed, fit_seed, \
  ...
  if ( dp_version != '1.3.3' ):
    deepmd_param['training'].pop('load_ckpt')

  if ( iter_id > 0 and use_prev_model ):
    deepmd_prev_inp_file = ''.join((work_dir, '/iter_', str(iter_id-1), '/01.train/0/input.json'))
    with open(deepmd_prev_inp_file, 'r') as f:
      deepmd_prev_dic = json.load(f)
    start_lr_prev = deepmd_prev_dic['learning_rate']['start_lr']

    if ( start_lr_prev/lr_scale < start_lr/100.0 ):
      deepmd_param['learning_rate']['start_lr'] = start_lr/(100.0)
    else:
      deepmd_param['learning_rate']['start_lr'] = start_lr_prev/lr_scale
