...
def gen_cp2kfrc_file(cp2k_param, work_dir, iter_id, sys_id, task_id, coord, box, train_stress):
  ...
  if ( len(coord) != 0 ):
    ...
    std_inp_file.write('    &PRINT\n')
    std_inp_file.write('      &ATOMIC_COORDINATES\n')
    std_inp_file.write('        FILENAME\n')
    std_inp_file.write('      &END ATOMIC_COORDINATES\n')
    ...
    revise_cp2k_inp.delete_line('SCF_GUESS', std_inp_file_name_abs, cp2k_sys_task_dir)
    ...
    scf_line_num = file_tools.grep_line_num('&SCF', cp2k_inp_file_upper, cp2k_sys_task_dir)[0]
    call.call_simple_shell(cp2k_sys_task_dir, 'rm %s' %(cp2k_inp_file_upper))
    ...
    for i in range(len(coord)):
      ...
      if use_prev_wfn:
        if ( i != 0 ):
          ...
          cmd = "sed -i '%d s/^/    WFN_RESTART_FILE_NAME .\/cp2k-RESTART.wfn\\n/' input.inp" %(pot_line_num+1)
      else:
        cmd = "sed -i '%d s/^/    WFN_RESTART_FILE_NAME .\/cp2k-RESTART.wfn\\n/' input.inp" %(pot_line_num+1)
      call.call_simple_shell(cp2k_sys_task_traj_dir, cmd)
      if ( pot_line_num < scf_line_num ):
        cmd = "sed -i '%d s/^/      SCF_GUESS  RESTART\\n/' input.inp" %(scf_line_num+2)
      elif ( i==0  and pot_line_num < scf_line_num ):
        cmd = "sed -i '%d s/^/      SCF_GUESS  RESTART\\n/' input.inp" %(scf_line_num+1)
      call.call_simple_shell(cp2k_sys_task_traj_dir, cmd)
