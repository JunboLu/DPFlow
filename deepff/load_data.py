...
def load_data_from_sepfile(file_dir, save_dir, file_prefix, proj_name, tot_atoms_type_dic, choosed_index, out_file_name='cp2k.out'):
  ...
  for i in sorted(choosed_index):
    ...
    box_file_i = ''.join((task_dir_i, '/', proj_name, '-1.Log')) 
    coord_file_i = ''.join((task_dir_i, '/', proj_name, '-1.coordLog'))
    ...
    if ( box_exist and coord_exist and out_exist and frc_exist ):
      ...
      if ( len(grep_scf_info) == 0 ):
        vec = []
        for j in range(3):
          line_ij = linecache.getline(box_file_i, j+3)
          line_ij_split = data_op.split_str(line_ij, ' ', '\n')
          #vol will be used in stress calculation.
          vec.append([float(line_ij_split[4]), \
                      float(line_ij_split[5]), \
                      float(line_ij_split[6])])
          if ( j== 0 ):
            frame_str = ' '.join((line_ij_split[4], line_ij_split[5], line_ij_split[6]))
          else:
            frame_str = ' '.join((frame_str, line_ij_split[4], line_ij_split[5], line_ij_split[6]))
      ...
      log_info.log_error('The output file or force file may not exist in task %d in %s' %(i, task_dir_i))
      exit()
  box_file.close()
  frc_file.close()
  coord_file.close()
...
def load_data_from_dir(traj_coord_file_name, traj_frc_file_name, traj_cell_file_name, traj_stress_file_name, \
  ...
  if ( start < start_id ):
    log_info.log_error('Input error: start is less than the starting frame id in trajectory %s, please check or reset deepff/deepmd/training/system/start' %(traj_coord_file_name))
    exit()

  if ( end > end_id ):
    log_info.log_error('Input error: end is larger than the endding frame id in trajectory %s, please check or reset deepff/deepmd/training/system/end' %(traj_coord_file_name))
    exit()

  if ( start >= end ):
    log_info.log_error('Input error: start_frame is larger than end_frame in trajectory %s, please check!' %(traj_coord_file_name))
    exit()
  else:
    total_index = data_op.gen_list(start, end, each)
    max_choosed_num = int((end-start)/each)+1
    if ( choosed_num > max_choosed_num ):
      log_info.log_error('Input error: choosed_frame_num is larger than the number of frames in trajectory %s, please check!' %(traj_coord_file_name))
      exit()
    else:
  ...
  if ( no_train_data_index != [] ):
    ...
    if ( not os.path.exists(no_train_data_dir) ):
...
#  file_dir = '/home/lujunbo/code/github/DPFlow/deepff/work_dir/iter_1/03.cp2k_calc/cp2k_task_1'
#  file_prefix = 'task_'
#  proj_name = 'cp2k'
...
#  load_data.raw_to_set(file_dir + '/data', 1)
  ...
  save_dir = '/home/lujunbo/WORK/Deepmd/DPFlow/co2_metad/iter_1/03.cp2k_calc/sys_0/data'
