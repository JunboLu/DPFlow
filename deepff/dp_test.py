...
def supervised_test(cp2k_pos_file, cp2k_cell_file, cp2k_frc_file, lmp_exe, \
                    lmp_mpi_num, lmp_omp_num, dpff_file, atom_label, work_dir):
...
def active_learning_test(work_dir, iter_id, atoms_type_multi_sys, use_bias_tot, success_force_conv, max_force_conv, energy_conv):
    ...
    for j in range(task_num):
      ...
      if use_bias_tot[i]:
        ...
        for k in range(frames_num):
          ...
          log_file = ''.join((model_traj_dir, '/lammps.out'))
          ...
          linecache.clearcache()
          step_line_num = file_tools.grep_line_num('Step ', log_file, model_traj_dir)
          #print(step_line_num)
          line = linecache.getline(log_file, int(step_line_num[0]) + 1)
          line_split = data_op.split_str(line, ' ', '\n')
          energy_lmp.append(float(line_split[2]))
          linecache.clearcache()
