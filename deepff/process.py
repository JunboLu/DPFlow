...
def dump_init_data(work_dir, deepmd_dic, train_stress, tot_atoms_type_dic, restart_iter, restart_stage):
  ...
  for key in train_dic:
    if ( 'system' in key):
      ...
      if ( all(j == choosed_num for j in data_num) ):
        ...
        if ( restart_iter == 0 and restart_stage == 0 ):
          traj_type = train_dic[key]['traj_type']
          ...
          if ( traj_type == 'unbias' ):
            ...
            load_data.load_data_from_dir(traj_coord_file, traj_frc_file, traj_cell_file, \
                                         traj_stress_file, train_stress, work_dir, save_dir, \
                                         start, end, choosed_num, tot_atoms_type_dic)
          elif ( traj_type == 'bias' ):
            data_dir = train_dic[key]['data_dir']
            data_0_inp = ''.join((data_dir, '/task_0/cp2k.inp'))
            if ( os.path.exists(data_0_inp) ):
              proj_name = revise_cp2k_inp.get_proj_name(data_0_inp, work_dir) 
            else:
              log_info.log_error('Running error: % does not exist, the force calc for bias md fail' %(data_0_inp))
              exit()
            out_file_name = train_dic[key]['out_file_name']
            max_choosed_num = end-start+1
            if ( choosed_num > max_choosed_num ):
              log_info.log_error('Input error: choosed_frame_num is larger than the number of frames in trajectory in %s, please check!' %(data_dir))
              exit()
            ...
            load_data.load_data_from_sepfile(data_dir, save_dir, 'task_', proj_name, tot_atoms_type_dic, \
          ...
          init_data_num_part, init_test_data_num_part = \
          load_data.raw_data_to_set(parts, shuffle_data, save_dir, energy_array, \
                                    coord_array, frc_array, box_array, virial_array)
        else:
          log_info.log_error('Running error: the init_train_data has been changed, you must start from scratch!')
          exit()

      init_data_num.append(init_data_num_part)
      i = i+1

  for key in train_dic: 
    if ( 'new_data_dir' in key ):
      init_train_data.append(os.path.abspath(os.path.expanduser(train_dic[key])))
      energy_npy_file = ''.join((os.path.abspath(os.path.expanduser(train_dic[key])), '/set.000/energy.npy'))
      new_data_num = len(np.load(energy_npy_file))
      init_data_num.append(new_data_num)

  return init_train_data, init_data_num

def check_deepff_run(work_dir, iter_id, dp_version):
  ...
  for i in range(model_num):
    ...
    for j in range(choosed_line_num):
      ...
      if ( data_op.eval_str(line_split[0]) == 1 and len(line_split) >= max_print_num ):
        force_trn.append(float(line_split[force_id]))
    linecache.clearcache()
