...
def power_spectrum_mode(atoms_num, pre_base_block, end_base_block, pre_base, each, start_frame_id, \
  ...
  print (data_op.str_wrap(str_print, 80), flush=True)
  str_print = 'The velocity-velocity auto correlation function of mode 3 (anti-symmetric strech) is written in %s' %(tcf_q3_file)
  print (data_op.str_wrap(str_print, 80), flush=True)
  ...
  if ( spec_type == 'general' ):
    ...
    if ( spec_type == 'water_mode' ):
      ...
      if (start_frame_id_v > start_frame_id_p):
        atoms_num, pre_base_block, end_base_block, pre_base, frames_num, \
        each, start_frame_id, end_frame_id, time_step = \
        traj_info.get_traj_info(traj_vel_file, 'vel')
      else:
        atoms_num, pre_base_block, end_base_block, pre_base, \
        frames_num, each, start_frame_id, end_frame_id, time_step = \
        traj_info.get_traj_info(traj_coord_file, 'coord_xyz')
    ...
    print (data_op.str_wrap(str_print, 80), flush=True)
    str_print = 'The power spectrum of mode 3 (anti-symmetric) is written in %s' %(freq_int_q3_file)
    print (data_op.str_wrap(str_print, 80), flush=True)
